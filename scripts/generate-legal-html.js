...
4. 写真（端末内のみ）
   記録に添付した写真は端末内（アプリ専用領域）にのみ保存され、外部サーバーへ送信されません。機種変更時には引き継がれません。

■ 外部サービスへの送信（LifeTrace）

本アプリは、経路や地図の表示のため以下の外部サービスへ通信します。いずれも広告・トラッキング目的には使用しません。

・経路検索API（api.transit.ls8h.com）: バス・船の経路を補完する際に、出発地と到着地の緯度経度を送信します。氏名・メールアドレス等は送信しません。
・地図タイル配信（CARTO / OpenFreeMap / MapTiler）: 地図の表示に必要なタイル画像を取得します。表示中の地図の範囲と接続元IPアドレスが各サービスに送信されます。
・ウィキメディア（commons.wikimedia.org / ja.wikipedia.org）: 都道府県の紹介画像を取得します。`;
